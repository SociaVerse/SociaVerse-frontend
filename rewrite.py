...
with open('D:/SociaVerse/SociaVerse-frontend/sociaverse-frontend/components/post-card.tsx', 'r', encoding='utf-8') as f:
    text = f.read()
            ...
            {post.images && post.images.length > 0 && (
                <div className="relative flex flex-col group/image">
                    {post.images.map((imgObj, index) => {
                        const imgUrl = imgObj.image.startsWith('http') ? imgObj.image : ${process.env.NEXT_PUBLIC_API_URL};
                        return (
                            <Image
                                key={imgObj.id || index}
                                src={imgUrl}
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 md:group-hover/image:opacity-100 hover:opacity-100 transition duration-300 flex items-center justify-center gap-4 z-10 pointer-events-auto">
                        <Button
                            variant="ghost"
                            size="icon"
                    ...
                    <div className="flex items-center gap-3 overflow-hidden">
                        <Link href={/u/} onClick={(e) => e.stopPropagation()} className="flex-shrink-0">
                            {post.author.profile_picture ? (
                                <img src={post.author.profile_picture.startsWith('http') ? post.author.profile_picture : ${process.env.NEXT_PUBLIC_API_URL}} alt={post.author.username} className="w-8 h-8 rounded-full object-cover hover:opacity-90 transition-opacity" />
                            ) : (
...
with open('D:/SociaVerse/SociaVerse-frontend/sociaverse-frontend/components/post-card.tsx', 'w', encoding='utf-8') as f:
    f.write(text[:start_idx] + new_return + text[end_idx:])
print('DONE')
